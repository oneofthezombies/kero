...
class ParserImpl : public ::KeroGrammarParserAuxil {
public:
  ParserImpl(const std::string_view source) noexcept : source_{source} {
    SetPccCallbacks();
    context_ = ::KeroGrammarParser_create(this);
  }

  ~ParserImpl() noexcept {
    ::KeroGrammarParser_destroy(context_);
    context_ = nullptr;
  }
  ...
  auto Parse() noexcept -> bool {
    ...
    while (true) {
      ...
      if (::KeroGrammarParser_parse(context_, &ret) == 0) {
        break;
      }
